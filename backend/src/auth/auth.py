...
AUTH0_DOMAIN = 'moehab.eu.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'Caffee'
...
def check_permissions(permission, payload):
    # Check if 'permissions' is included in the payload
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'permissions_key_not_found',
            'description': 'Permissions is not included in the payload'
        }, 401)

    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'unauthorized',
            'description': 'The logged-in user does not have the privilege to use this method'
        }, 401)
